        ...
        public List<User> retrieveUser(JSON filter)
        {
            ...
            for (int i = 0; i < result.Count; i++)
            {
                ...
                if (data["managerId"].ToString() == "")
                {
                    ...
                    {
                        user = new Club(_uid, data["email"].ToString(), data["password"].ToString(), auth, 0, null, 0, "", "", 0, null, null);
                    }
                    else if (clientType == UserType.Player)
                    {
                        user = new Player(_uid, data["email"].ToString(), data["password"].ToString(), auth, 0, null, -1, "", "", "", 0, "", 0, 0, 0, "", null);
                    }
                    else
                    {
                        continue;
